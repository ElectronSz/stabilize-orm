...
import { DBType, RelationType, StabilizeError, type CacheConfig, type DBConfig } from './types';
import { ModelKey, ColumnKey, ValidatorKey, RelationKey, SoftDeleteKey } from './decorators';
...
export class Repository<T> {
  ...
  private relations: Record<string, { type: RelationType; targetModel: () => any; foreignKey?: string; inverseKey?: string; joinTable?: string }>;
  private softDeleteField: string | null;
  private logger: Logger;

  constructor(client: DBClient, model: new (...args: any[]) => T, cacheConfig: CacheConfig = { enabled: false, ttl: 60 }, logger: Logger = new ConsoleLogger()) {
    this.client = client;
    ...
    this.softDeleteField = Reflect.getMetadata(SoftDeleteKey, model.prototype) || null;
    this.logger = logger;
  }
  ...
  async findOne(id: number | string, options: { relations?: string[] } = {}): Promise<T | null> {
    ...
    const cacheKey = options.relations ? `findOne:${this.table}:${id}:${options.relations.join(',')}` : `findOne:${this.table}:${id}`;
    const results = await queryBuilder.execute(this.client, this.cache!, cacheKey);
    this.logger.logDebug(`Found ${this.table} with ID ${id} in ${(performance.now() - start).toFixed(2)}ms`);
    return results[0] || null;
  }

  async create(entity: Partial<T>, options: { relations?: string[] } = {}): Promise<T> {
        const start = performance.now();
        this.logger.logDebug(`Creating ${this.table} with data: ${JSON.stringify(entity)}`);
        ...
        id = (await this.client.query<{ id: number }>('SELECT last_insert_rowid() as id', []))[0]?.id;
        } else if (dbType === DBType.MySQL) {
        id = (await this.client.query<{ id: number }>('SELECT LAST_INSERT_ID() as id', []))[0]?.id;
        }
        }
        ...
        const result = (insertedResult?.[0] || (await queryBuilder.execute(this.client))[0]) as T;
        await this.cache.set(`findOne:${this.table}:${id}`, [result], 60);
        this.logger.logDebug(`Created ${this.table} with ID ${id} in ${(performance.now() - start).toFixed(2)}ms`);
        return result;
        }
        }
  ...
  async bulkCreate(entities: Partial<T>[], options: { relations?: string[]; batchSize?: number } = {}): Promise<T[]> {
    const start = performance.now();
    this.logger.logDebug(`Bulk creating ${entities.length} ${this.table} entities`);
    ...
    for (let i = 0; i < entities.length; i += batchSize) {
      ...
      await this.client.transaction(async () => {
        ...
        const params = batch.flatMap(entity => keys.map(k => (entity as any)[k]));
        ...
        if (dbType === DBType.Postgres) {
            ...
            ids = (await this.client.query<{ id: number }>(
                `SELECT id FROM ${this.table} ORDER BY id DESC LIMIT ?`,
                [batch.length]
            )).map(row => row.id);
        }

        const cacheKeys = ids.map(id => `findOne:${this.table}:${id}`).concat(`find:${this.table}`);
        
        // If batchResults is empty (non-PostgreSQL), we need to fetch the full records.
        if (ids.length > 0 && batchResults.length === 0) {
            const queryBuilder = this.find().where(`id IN (${ids.map(() => '?').join(', ')})`, ...ids);
            if (options.relations) {
              for (const rel of options.relations) {
                await this.loadRelation(queryBuilder, rel);
              }
        ...
        if (this.cache) {
          await this.cache.invalidate(cacheKeys);
          if (this.cache.getStrategy() === 'write-through') {
            await this.cache.set(`bulkCreate:${this.table}:${ids.join(',')}`, batchResults, 60);
          }
        }
        results.push(...batchResults);
      });
    }
    this.logger.logDebug(`Bulk created ${results.length} ${this.table} entities in ${(performance.now() - start).toFixed(2)}ms`);
    return results;
  }
 ...
 async update(id: number | string, entity: Partial<T>): Promise<T> {
    ...
    return await this.client.transaction(async () => {
      ...
      if (this.cache) {
        ...
        if (this.cache.getStrategy() === 'write-through') {
          ...
          await this.cache.set(`findOne:${this.table}:${id}`, [result], 60);
          this.logger.logDebug(`Updated ${this.table} with ID ${id} in ${(performance.now() - start).toFixed(2)}ms`);
          return result;
        }
      }
  ...
  async bulkUpdate(updates: { where: { condition: string; params: any[] }; set: Partial<T> }[], options: { batchSize?: number } = {}): Promise<void> {
    const start = performance.now();
    this.logger.logDebug(`Bulk updating ${updates.length} ${this.table} entities`);
    ...
    for (let i = 0; i < updates.length; i += batchSize) {
      ...
      await this.client.transaction(async () => {
        for (const update of batch) {
          ...
          const params = [...keys.map(k => (update.set as any)[k]), ...update.where.params];
          await this.client.query(query, params);
        }
        if (this.cache) {
    ...
    this.logger.logDebug(`Bulk updated ${updates.length} ${this.table} entities in ${(performance.now() - start).toFixed(2)}ms`);
  }

async upsert(entity: Partial<T>, keys: string[]): Promise<T> {
    const start = performance.now();
    this.logger.logDebug(`Upserting ${this.table} with keys: ${keys.join(', ')}`);
    this.validate(entity);
    return await this.client.transaction(async () => {
      const dbType = this.getDBType();
      ...
      const updateClause = columns.filter(c => !keys.includes(c)).map(c => `${this.columns[c]?.name} = ?`).join(', ');
      ...
      const params = [...columns.map(k => (entity as any)[k]), ...columns.filter(c => !keys.includes(c)).map(k => (entity as any)[k])];
      ...
      if (this.cache) {
        ...
        if (this.cache.getStrategy() === 'write-through') {
          ...
          await this.cache.set(`findOne:${this.table}:${id}`, [result], 60);
          this.logger.logDebug(`Upserted ${this.table} with ID ${id} in ${(performance.now() - start).toFixed(2)}ms`);
          return result;
        }
      }
  ...
  async delete(id: number | string): Promise<void> {
    ...
    if (this.softDeleteField) {
      await this.client.query(`UPDATE ${this.table} SET ${this.softDeleteField} = ? WHERE id = ?`, [new Date().toISOString(), id]);
    } else {
      await this.client.query(`DELETE FROM ${this.table} WHERE id = ?`, [id]);
    }
    if (this.cache) {
      await this.cache.invalidate([`find:${this.table}`, `findOne:${this.table}:${id}`]);
    }
    this.logger.logDebug(`Deleted ${this.table} with ID ${id} in ${(performance.now() - start).toFixed(2)}ms`);
  }

  async bulkDelete(ids: (number | string)[], options: { batchSize?: number } = {}): Promise<void> {
    const start = performance.now();
    ...
    for (let i = 0; i < ids.length; i += batchSize) {
      ...
      await this.client.transaction(async () => {
        ...
        const params = this.softDeleteField ? [new Date().toISOString(), ...batch] : batch;
        await this.client.query(query, params);
      });
    }
    if (this.cache) {
    ...
    this.logger.logDebug(`Bulk deleted ${ids.length} ${this.table} entities in ${(performance.now() - start).toFixed(2)}ms`);
  }
  ...
  async recover(id: number | string): Promise<T> {
    ...
    if (!this.softDeleteField) {
      throw new StabilizeError('Soft delete not enabled for this model', 'RECOVER_ERROR');
    }
    return await this.client.transaction(async () => {
      ...
      this.logger.logDebug(`Recovered ${this.table} with ID ${id} in ${(performance.now() - start).toFixed(2)}ms`);
      return result;
    });
  }
  ...
  async rawQuery<T>(query: string, params: any[] = []): Promise<T[]> {
    ...
    this.logger.logDebug(`Raw query completed in ${(performance.now() - start).toFixed(2)}ms`);
    return result;
  }
  ...
  private async loadRelation(queryBuilder: QueryBuilder<T>, relation: string) {
    ...
    if (!rel) throw new StabilizeError(`Relation ${relation} not found`, 'RELATION_ERROR');
    ...
    if (rel.type === RelationType.OneToOne || rel.type === RelationType.ManyToOne) {
      queryBuilder.join(relatedTable, `${this.table}.${rel.foreignKey} = ${relatedTable}.id`);
    } else if (rel.type === RelationType.OneToMany) {
      queryBuilder.join(relatedTable, `${relatedTable}.${rel.inverseKey} = ${this.table}.id`);
